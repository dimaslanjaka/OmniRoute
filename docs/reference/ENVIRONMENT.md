...
| `ENABLED_PROVIDERS`                     | _(unset — all providers enabled)_                               | `src/shared/utils/providerFilter.ts`        | Comma-separated provider allowlist. Wildcards (`*`) supported (e.g. `openai-compatible-*`). When set, only matching providers are exposed in constants, registry, executors and model listings. See [`docs/guides/CUSTOM_BUILDS.md`](../guides/CUSTOM_BUILDS.md).                                 |
